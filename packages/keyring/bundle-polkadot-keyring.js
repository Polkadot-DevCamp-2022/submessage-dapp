const polkadotKeyring = (function (exports, util, utilCrypto) {
  ...
  function createPair({
    ...
    return {
      ...
      decrypt: encryptedMessage => {
        util.assert(!isLocked(secretKey), 'Cannot decrypt with a locked key pair');
        util.assert(!['ecdsa', 'ethereum'].includes(type), 'Secp256k1 not supported yet');
        return type === 'ed25519' ? utilCrypto.ed25519Decrypt(util.u8aToU8a(encryptedMessage), {
          publicKey,
          secretKey
        }) : utilCrypto.sr25519Decrypt(util.u8aToU8a(encryptedMessage), {
          publicKey,
          secretKey
        });
      },
      ...
      encrypt: (message, recipientPublicKey) => {
        util.assert(!isLocked(secretKey), 'Cannot encrypt with a locked key pair');
        util.assert(!['ecdsa', 'ethereum'].includes(type), 'Secp256k1 not supported yet');
        return utilCrypto.encrypt(message, recipientPublicKey, type, {
          publicKey,
          secretKey
        });
      },
  ...
  class Keyring {
    ...
    encrypt(message, recipientPublicKey, recipientKeyType) {
      return utilCrypto.encrypt(message, recipientPublicKey, recipientKeyType || this.type);
    }
  }
